...
> - [fig_modules_expand.html](figures/fig_modules_expand.html)——**展开图**（模块明细与接入点）：四个彩色区域=四方归属，把上游项目的主要模块全部展开——vLLM 的 Scheduler/KVCacheManager/KVConnector 框架/（不启动的）EngineCore·ModelRunner·Worker；Ascend 插件的 BalanceScheduler 补丁/（未启用的）Recompute·torch_npu 层；Conductor 的 PrefixCacheTable·HashStrategy/（未引入的）Router·TransferEngine·Store。每个模块的 tag 标注处置（真实执行/不启动/未启用/未引入），左列六个自研 mock 用粗边连到**它们各自接入或替换的那个上游模块**。
>
