...
> **主视图（先看这两张）**：
> - [fig_logic_owners.html](figures/fig_logic_owners.html)——**逻辑图**（泳道=三方归属）：横向读请求流程（到达→路由→组批→命中→逐层加载/计算→首 token），纵向看归属——每一步落在哪条泳道，就属于谁：**Mooncake Conductor【真实 C++】**（两次前缀索引查询）、**vLLM + Ascend 插件【真实代码】**（schedule 组批/块分配/停止检查）、**lab 仿真【自研 mock】**（trace 输入、路由打分、加载计划与逐层 IO、计算时刻）。
> - [fig_modules_expand.html](figures/fig_modules_expand.html)——**展开图**（三条归属带，与逻辑图泳道一一对应）：每条带展开该方的核心模块并标注取码方式与处置——**Conductor 带**（仅取 prefixindex 子集 10 个 C++ 文件编译；Router/TransferEngine/Store 未引入）、**lab 带**（六个自研 mock，"接口归上游、硬件执行归仿真"）、**vLLM+Ascend 带**（vLLM **整包 vendor** 全量源码按需 import；Ascend 仅取 3 个文件；EngineCore/ModelRunner/Worker/torch_npu 不启动/未启用）。粗边=mock 接入点，虚线边=角色替换。
>
> 过渡视图：[fig_arch_layers.html](figures/fig_arch_layers.html)（自上而下分层架构）· [fig_flow_request.html](figures/fig_flow_request.html)（泳道流程图）；细节视图：[fig_arch_platform.html](figures/fig_arch_platform.html)（组件图）· [fig_seq_lifecycle.html](figures/fig_seq_lifecycle.html)（时序图）
...
### 2.0 取码方式：完整 vendor 还是抠子集？

| 项目 | 取码方式 | 说明 |
|---|---|---|
| vLLM v0.20.2 | **整包完整源码** | 从 0.20.2 wheel 拷贝**全部** Python 文件到 `vendor/vllm/`（2279 条哈希锁定，与上游 tag 逐字节一致）。不用的模块（EngineCore/ModelRunner/Worker 等）**同样在包里，只是从不 import**——需要哪个模块就导入哪个 |
| vLLM-Ascend rc1 | **抠 3 个文件** | 仅 `platform.py`、`patch/platform/patch_balance_schedule.py`、`__init__.py`；整包导入会拉起 torch_npu/CANN 设备链，故由 `ascend_loader.py` 对补丁文件做 AST 节点选择执行 |
| Mooncake `fd72779` | **抠 10 个 C++ 文件** | 仅 prefixindex 子集（`hash_strategy`、`prefix_indexer` 及公共头），与桥接层一起编译为 `libconductor_lab.so`；router/TransferEngine/Store 属上游完整版，未引入 |

### 2.0b 为什么全局路由 / KV 加载 / 本层计算放在 lab 侧而不是上游？

这三件事不是"从上游挪出来的"，而是**上游要么没有对应物、要么对应物属于需要真实硬件的执行面**：

| lab 侧环节 | 上游的对应物 | 放在 lab 侧的原因 |
|---|---|---|
| 全局路由打分 | 无。Conductor 公开代码只有**索引器**（官方架构里 router 查询索引后自选实例，论文级 router 未开源）；vLLM 是单实例引擎，无跨实例路由 | 上游无码可跑，这正是研究挂载点：若未来取得真实 router，应替换 `run.py arrive()` 并保留 adapter |
| KV 加载与逐层 IO | vLLM 的 KVConnector **只有接口**（`KVConnectorBase_V1` 钩子），官方实现（NIXL/TorchNPU 等）依赖真实网络与远端存储 | 我们实现 `SimConnector` 填充同一接口：接口与调用时序归 vLLM，传输物理时序归 Storage 流体模型（CPU 上无法真传） |
| 本层计算 | vLLM ModelRunner/Worker 的真实 forward（需要 NPU/GPU） | 真实代码决定"算什么、算多少"（SchedulerOutput），仿真决定"何时完成"（`compute_time()` 解析公式）——这是控制面/执行面切分原则的直接体现 |
...
**Router 现状的实证核查（2026-09-23）**：锁定提交与当前 HEAD 的 `mooncake-conductor/src`、`include/conductor` 均只有 4 个子目录（`common`、`kvevent`、`prefixindex`、`zmq`）；全仓库递归树中**没有任何路径含 "router"**；路线图 issue #2189 仍为 open（2026-09-07 尚在讨论 Phase 3 事件订阅/并发查询），论文级 router 只存在于 RFC #977 设计文档。`kvevent`（KV 事件订阅）与 `zmq` 是"喂给 router 的数据管道"，router 本体未合入。

**与 NVIDIA Dynamo 的对标关系**：概念上，Conductor（论文中的全局调度器，按 KVCache 分布做缓存感知路由）对标 Dynamo 的 **KV-aware Router**（prefix cache match ratio + decode cost 打分），Mooncake Transfer Engine 对标 NIXL；差异是 Dynamo 为组件中立的可插拔 serving 框架、Mooncake 为 KVCache 中心的一体化平台（Kimi 生产），且两者正在融合（Dynamo KVBM + Mooncake Store 集成 RFC，2026-04）。值得注意：**Dynamo 的 KV Router 是开源的**——若研究需要"真实 router 代码"作基线或对照组，它是可获得的目标（可挂到 `run.py arrive()` 的挂载点上）。

**开源现状 vs RFC 全貌（2026-09-23 实证）**：主线 `mooncake-conductor` 只有 4 个 C++ **库**组件（CMake 仅 `add_library`，无服务进程；旧 Go 服务 `conductor-ctrl/main.go` 已不在主线）——`prefixindex`（前缀索引本体）、`kvevent/object_key_parser`（把 vLLM/SGLang 对象键解析为索引条目）、`zmq`（订阅引擎 KV 事件流的客户端与解码）、`common`。即**只开源了 Router 的数据面原语**（"哪个前缀在哪"+事件管道）。对照 [RFC #977](https://github.com/kvcache-ai/Mooncake/issues/977) 的完整蓝图，尚缺：① 服务面（常驻进程 + REST/gRPC 的 Register/Unregister/Query，路线图 Phase 1–2 在做）；② 状态收集面（实例注册、GPU 利用率/队列等负载指标——ZMQ 事件流只覆盖 KV 分布）；③ **决策面（核心缺口）**：Prefill 调度器（缓存局部性×负载的成本函数选实例，RFC 明言灵感来自 Dynamo 智能路由与 SGLang Router）、Decode 调度器（负载打分 + TBT SLO 预测 + 早期拒绝）、后台数据调度器（热度分级与热点迁移）；④ 与 Transfer Engine 的传输编排。路线图四期均无路由决策内容。**本工程的 `run.py arrive()` 打分（队列 token×线性系数 + `estimated_io` 命中估值）恰是 RFC Prefill 成本函数的极简同构版**——若 RFC 落地，可整体替换为官方实现做对照。
