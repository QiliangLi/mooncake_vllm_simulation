...
uv pip install --offline --python .venv/bin/python 'torch==2.8.0+cpu' --index-url https://download.pytorch.org/whl/cpu || \
...
uv pip install --python .venv/bin/python --no-deps 'vllm==0.11.0' 'xgrammar==0.1.25'
