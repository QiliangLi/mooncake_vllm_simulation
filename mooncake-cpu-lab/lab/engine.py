...
import torch
from vllm.config import CacheConfig, SchedulerConfig, KVTransferConfig, ObservabilityConfig
...
class SimConnector(KVConnectorBase_V1):
    def __init__(self, cfg, kv_cache_config, conductor, worker_name, mode):
        super().__init__(cfg, KVConnectorRole.SCHEDULER, kv_cache_config)
...
def scheduler_class(name):
    ...
    from .ascend_loader import load_scheduler
...
def make_scheduler(cfg, kind):
    ...
    sc = SchedulerConfig(max_num_seqs=cfg['max_num_seqs'],
        ...
        max_model_len=cfg['max_model_len'], is_encoder_decoder=False,
        enable_chunked_prefill=cfg['chunked_prefill'],
    ...
    vc = NS(scheduler_config=sc, cache_config=cc, lora_config=None, kv_events_config=None,
            parallel_config=NS(data_parallel_index=0, decode_context_parallel_size=1,
                prefill_context_parallel_size=1, pipeline_parallel_size=1),
            model_config=NS(is_encoder_decoder=False, is_multimodal_model=False,
                max_model_len=cfg['max_model_len'], enable_return_routed_experts=False),
            observability_config=ObservabilityConfig(), additional_config={},
    ...
    return scheduler_class(kind)(vc, kv, NoStructuredOutput(), block_size=bs,
                                 hash_block_size=bs, log_stats=False), vc
...
class Engine:
    def __init__(self, sim, wid, kind):
        ...
        # Inject the virtual connector after construction instead of launching a
        # real transfer backend via KVConnectorFactory.
        vc.kv_transfer_config = KVTransferConfig(kv_connector='SimConnector', kv_role='kv_consumer')
        self.connector = SimConnector(vc, self.scheduler.kv_cache_config,
                                      sim.conductor, f'w{wid}', self.cfg['load_mode'])
    ...
    def add(self, job):
        request = Request(job['id'], job['tokens'], SamplingParams(
            ...
            pooling_params=None, arrival_time=job['arrival_s'], block_hasher=self.hasher)
    ...
    def runner_output(self, so, completed=()):
        ...
        return ModelRunnerOutput(req_ids=ids, req_id_to_index={r:i for i,r in enumerate(ids)},
            sampled_token_ids=sampled,
            kv_connector_output=KVConnectorOutput(finished_recving=set(completed)) if completed else None)
    ...
    def step(self):
        ...
        self.sim.emit('schedule', worker=self.wid, tokens=dict(so.num_scheduled_tokens),
            ...
            skipped_waiting=[r.request_id for r in self.scheduler.skipped_waiting],
