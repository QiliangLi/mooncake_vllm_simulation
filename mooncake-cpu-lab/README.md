...
实际运行的是 Mooncake 的 C++ `PrefixCacheTable`、vLLM V1 `Scheduler` 或 vLLM-Ascend `AscendScheduler`、真实 `KVCacheManager` / `BlockPool` / 请求状态机。全局路由策略、虚拟时钟、计算和共享存储模型由本工程实现。
...
# 真实 AscendScheduler；这个配置开启 chunked prefill，会复用其父类调度路径。
.venv/bin/python -m lab.run --engine ascend --out results/ascend-layerwise

# 执行 Ascend 独有的 prefill-first 分支。
.venv/bin/python -m lab.run --engine ascend \
  --config configs/ascend-prefill-first.json --out results/ascend-prefill-first
...
第一次安装需要联网获取 Python 包。安装 vLLM 的 wheel 是为了获得准确的版本元数据和依赖文件；显式 `--no-deps` 避免拉取整套 CUDA 运行库。这里的 Python 依赖子集仅支持控制层实验，不构成一个可提供真实推理的 vLLM 安装。
...
| 改 Ascend prefill-first 等真实调度代码 | `vendor/vllm_ascend/core/scheduler.py` |
