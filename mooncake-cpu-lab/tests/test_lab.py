...
def test_conductor_hash_matches_real_vllm():
    ...
    r=Request('hash',tokens,SamplingParams(max_tokens=1),None,
...
def test_ascend_unchunked_prefill():
    from lab.run import Simulation
    cfg=json.loads((ROOT/'configs/ascend-unchunked.json').read_text())
    sim=Simulation(cfg,jobs(),'ascend'); result=sim.run()
    assert result['requests']==12
    assert not sim.engines[0].scheduler.scheduler_config.enable_chunked_prefill
    assert type(sim.engines[0].scheduler).__name__=='BalanceScheduler'
    assert not sim.engines[0].scheduler._balance_enabled

def test_ascend_rc1_default_matches_vllm_without_device_startup():
    import sys
    from lab.run import Simulation
    a=Simulation(config(),jobs(),'vllm'); a.run()
    b=Simulation(config(),jobs(),'ascend'); result=b.run()
    assert a.events==b.events
    assert result['scheduler_class']=='BalanceScheduler'
    assert result['upstream']['vllm']['tag']=='v0.20.2'
    assert result['upstream']['vllm-ascend']['tag']=='v0.20.2rc1'
    assert 'torch_npu' not in sys.modules
    assert 'vllm.v1.engine.core' not in sys.modules
