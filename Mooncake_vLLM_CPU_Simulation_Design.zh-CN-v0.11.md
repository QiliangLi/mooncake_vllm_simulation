...
版本日期：2026-09-22。代码包：`mooncake-cpu-lab`。
...
| Ascend 本地调度 | `AscendScheduler`，包括 prefill-first 分支 | 是 |
...
| vLLM | `v0.11.0` | `b8b302cde434df8c9289a2b465406b47ebab1c2d` |
| vLLM-Ascend | `v0.11.0` | `2f1aed98ccdb0fcbe1ff4fd0abab225bfd8d0367` |
| PyTorch | `2.8.0+cpu` | 安装 CPU wheel |
| Python / C++ | Python 3.12 / C++20 | 验证环境 Python 3.12.14、g++ 13 |

vLLM / Ascend 0.11.0 是这份可重复起步工程的历史基线，**不是声称它们是当前最新版本**。选择它们是为了先把一组具体 API 与行为锁住并实测跑通。用于现网版本研究时，应换成现网匹配的 vLLM、Ascend、CANN 组合，重做接口和轨迹验证；不能只替换 `pip install` 版本号。
...
包内 `AscendScheduler` 是原始类。在当前固定版本，开启 chunked prefill 时其 `schedule()` 调用父类路径；关闭 chunked prefill 时执行 Ascend 的 prefill-first 分支。因此默认两种引擎得到相同调度轨迹是有代码原因的，不是漏接了 Ascend。

工程单独提供 `ascend-prefill-first.json` 并测试该非父类分支。通过模块路径直接加载此类，避免导入整个 NPU 插件启动链。
...
已完成的关键验证包括真实 C++ hash 与 vLLM block hash 相符、连续前缀查询与移除、共享带宽随流到达重新分配、两种引擎 × 两种加载模式、Ascend prefill-first、层 barrier、远端 KV 未到不调度、chunk 中间轮不产生 token、真实抢占 / 重计算、block 无泄漏、确定性回放、观察窗口守恒。
...
6. [vLLM 0.11.0 Scheduler 源码](https://github.com/vllm-project/vllm/blob/b8b302cde434df8c9289a2b465406b47ebab1c2d/vllm/v1/core/sched/scheduler.py)。
7. [vLLM 0.11.0 官方 scheduler 测试辅助](https://github.com/vllm-project/vllm/blob/b8b302cde434df8c9289a2b465406b47ebab1c2d/tests/v1/core/utils.py)：独立实例化 scheduler 的思路参考。
8. [KVConnectorBase_V1](https://docs.vllm.ai/en/v0.11.0/api/vllm/distributed/kv_transfer/kv_connector/v1/base.html)。
9. [SchedulerConfig](https://docs.vllm.ai/en/v0.11.0/api/vllm/config/scheduler.html)。
10. [AscendScheduler 固定源码](https://github.com/vllm-project/vllm-ascend/blob/2f1aed98ccdb0fcbe1ff4fd0abab225bfd8d0367/vllm_ascend/core/scheduler.py)。
